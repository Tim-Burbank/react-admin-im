...
import {fetchCustomerFinanceInfo,fetchCustomerTrans,updateCustomerFinanceInfo} from '../module/customerFinanceInfo'
import {formatDate,formatMoney,configDirectory,configDirectoryObject} from '../../../../../utils/formatData'




class CustomerFinanceInfo extends React.Component{
  constructor(props){
    super(props)
    this.state = {
      loading : false,
  ...
  componentWillMount(){
    ...
    if(location && location.query){
      ...
      dispatch(fetchCustomerFinanceInfo(params.id,location.query)).then((e)=>{
        if(e.error){
      ...
      dispatch(fetchCustomerTrans(json)).then((e)=>{
        if(e.error){
          message.error(e.error.message);
  ...
  onFetch = (values) =>{
    ...
    //获取数据
    dispatch(fetchCustomerFinanceInfo(params.id,values)).then((e)=>{
      if(e.error){
        message.error(e.error.message);
        this.setState({loading:false})
      }else{
  ...
  handleSave=()=>{
    const {dispatch,params,intl:{formatMessage},customerFinanceInfo} = this.props;
    const {capBalanceNum} = this.state
    let json = {
      capitalBalance:capBalanceNum
    }
    this.setState({modalLoad:true})
    dispatch(updateCustomerFinanceInfo(customerFinanceInfo.getIn(['wallet','id']),params.id,json)).then(e=>{
      console.log(e)
      if(e.error){
        message.error(e.error.message)
  ...
  render(){
    const {intl:{formatMessage},customerFinanceInfo,location:{pathname},customerFinanceTrans} = this.props;
    ...
    const columnMap=column=>{
      let text
      if(customerFinanceInfo){
        text=column.deep?customerFinanceInfo.getIn(column.deep):customerFinanceInfo.get(column.dataIndex)
      }else{
        text= ''
      }
      ...
      <Row>
        ...
        <Card {...cardProps}>
          ...
          {/*<Row type="flex" justify="center" style={{marginTop:"30px",marginBottom:"10px"}}>
            <Button onClick={()=>{this.setState({capModal:true,capBalanceNum:customerFinanceInfo.getIn(['wallet','capitalBalance'])})}}  type="primary" style={{marginRight:'20px'}} size="large">{formatMessage({id:'capBalance'})}</Button>
          </Row>*/}
        </Card>
        ...
        <ImmutableTable
          ...
          style={{marginTop:20}}
          columns={columns}
          dataSource={customerFinanceTrans}
          pagination={{ pageSize: 13,total:customerFinanceTrans&&customerFinanceTrans.size }}
        />
        </Spin>
        <Modal
...
CustomerFinanceInfo.propTypes = {
  pathJump : React.PropTypes.func,
};


const mapStateToProps = (state) => ({
  customerFinanceInfo : state.getIn(['customerFinanceInfo','customerFinanceInfo']),
  customerFinanceTrans:state.getIn(['customerFinanceInfo','customerFinanceTrans']),
});

export default injectIntl(connect(mapStateToProps)(CustomerFinanceInfo))
